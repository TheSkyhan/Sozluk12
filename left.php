...
?>
		...
		<tr>
			...
			text-align: center;" onclick="if (!show) {
                                ...
                                ;">h<br/>a<br/>y<br/>v<br/>a<br/>n<br/> <br/>a<br/>r<br/>a
		    </td>
		</tr>
	...
	$sql = "SELECT entry_baslik, toplam, max
				FROM (SELECT entry_baslik,entry_giristarihi,MAX(entry_iyi) AS max , COUNT( entry_baslik ) AS toplam
						...
						FROM tbl_entries 
						WHERE entry_baslik 
						REGEXP '[[:<:]]" . mysqli_real_escape_string($aramatxt) . "[[:>:]]'
						";
	if ($_GET["gün"] != null && $_GET["ay"] != null && $_GET["yıl"] != null) {
	    ...
	    $sql = $sql . " AND entry_giristarihi BETWEEN '" . mysqli_real_escape_string($tarih1) . "' AND '" . mysqli_real_escape_string($tarih2) . "'";
	}
	if ($_GET["yazar"] != null) {
	    $sql = $sql . " AND entry_yazar ='" . mysqli_real_escape_string($_GET["yazar"]) . "'";
	}
