    ...
    if (is_uploaded_file($_FILES['eksi']['tmp_name'])) {
        ...
        if (is_resource($zip)) {
            ...
            foreach ($xml->entries->entry as $entry) {
                $baslik = mysqli_real_escape_string($conn, $entry->attributes()->title->__toString());
                $zaman = mysqli_real_escape_string($conn, date("Y-m-d H:i:s", strtotime($entry->attributes()->date->__toString())));
                $entry_text = mysqli_real_escape_string($conn, $entry->__toString());
                $yazar = $_SESSION["nick"];

                if (!preg_match("/^\s*$/", $entry_text)) {
                    $sqlsirabul = "SELECT MAX(entry_sirano) AS sirano
                                   FROM tbl_entries
                                   WHERE entry_baslik = '" . $baslik . "'";

                    $snc = mysqli_query($conn, $sqlsirabul);

                    if (mysqli_num_rows($snc) == 0) {
                        $sirano = 1;
                    } else {
                        $row = mysqli_fetch_array($snc);
                        $sirano = $row["sirano"] + 1;
                    }
                    ...
                    $sql = "INSERT INTO
                            ...
                            VALUES (NULL, " . $sirano . ", '" . $baslik . "', '" . $entry_text . "', '" . $yazar . "', '" . $zaman . "', NULL, '', '', 1)";
                    mysqli_query($conn, $sql);
                    mysqli_query($conn, "DELETE FROM tbl_ukte WHERE entry_baslik='" . $baslik . "'");
                    mysqli_query($conn, "DELETE FROM tbl_kenar WHERE entry_baslik='" . $baslik . "' AND entry_yazar='" . $yazar . "' AND entry_text ='" . $entry_text . "'");
                }
            }
            zip_close($zip);
...
<form enctype="multipart/form-data" method="post">
    <table width="50%" border="0">
        ...
        <?php
        $sql = "SELECT COUNT(entry_id) AS toplam FROM tbl_entries WHERE entry_yazar = '" . mysqli_real_escape_string($conn, $yazar) . "'";
        ...
        $sql = "SELECT COUNT(entry_id) AS toplam FROM tbl_entries WHERE entry_yazar = '" . mysqli_real_escape_string($conn, $yazar) . "' AND entry_giristarihi BETWEEN '" . $biraystr . "' AND '" . $bugun . "'";
        $result = mysqli_query($conn, $sql);
        $row = mysqli_fetch_array($result);
        ...
        $sql = "SELECT COUNT(entry_id) AS toplam FROM tbl_entries WHERE entry_yazar = '" . mysqli_real_escape_string($conn, $yazar) . "' AND entry_giristarihi BETWEEN '" . $birhaftastr . "' AND '" . $bugun . "'";
        $result = mysqli_query($conn, $sql);
        $row = mysqli_fetch_array($result);
        ...
        $sql = "SELECT COUNT(entry_id) AS toplam FROM tbl_entries WHERE entry_yazar = '" . mysqli_real_escape_string($conn, $yazar) . "' AND entry_giristarihi BETWEEN '" . $bugunstr . "' AND '" . $bugun . "'";
        $result = mysqli_query($conn, $sql);
        $row = mysqli_fetch_array($result);
        $bugunToplam = $row["toplam"];

        $sql = "SELECT MAX(entry_giristarihi) AS sonentry FROM tbl_entries WHERE entry_yazar = '" . mysqli_real_escape_string($conn, $yazar) . "'";
        $result = mysqli_query($conn, $sql);
        $row = mysqli_fetch_array($result);
            ...
            <td class="left-menu">
                ...
                $sql = "SELECT entry_baslik, entry_id
                        FROM (
                            ...
                            FROM tbl_entries
                            WHERE entry_yazar = '" . mysqli_real_escape_string($conn, $yazar) . "'
            ...
            <td class="left-menu">
                ...
                $sql = "SELECT entry_baslik, entry_id, entry_iyi
                        FROM (
                            ...
                            FROM tbl_entries
                            WHERE entry_yazar = '" . mysqli_real_escape_string($conn, $yazar) . "' AND entry_iyi > 10
                            ORDER BY entry_giristarihi DESC
                        )tablo LIMIT 0,10";
            ...
            <td class="left-menu">
                ...
                $sql = "SELECT entry_baslik, entry_id, entry_kotu
                        FROM (
                            ...
                            FROM tbl_entries
                            WHERE entry_yazar = '" . mysqli_real_escape_string($conn, $yazar) . "' AND entry_kotu > 10
                            ORDER BY entry_giristarihi DESC
                        )tablo LIMIT 0,10";
