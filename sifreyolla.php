...
if (isset($_POST["gönder"])) {
    ...
    $sql = "SELECT user_pwd,user_email
				FROM tbl_users
				WHERE user_nick = '" . mysqli_real_escape_string($nick) . "' AND user_email ='" . mysqli_real_escape_string($mail) . "'";
	...
	$sql = "UPDATE tbl_users SET user_pwd = '" . md5($new_pwd) . "' WHERE user_nick = '" . mysqli_real_escape_string($nick) . "' AND user_email ='" . mysqli_real_escape_string($mail) . "'";
	$result = mysqli_query($conn, $sql);
