...
if (isset($_POST["gönder"])) {
	...
	$sql = "SELECT user_nick FROM tbl_users WHERE user_nick = '" . mysqli_real_escape_string($_POST["kime"]) . "'";
    ...
    VALUES (NULL ,  '" . mysqli_real_escape_string($_POST["nick"]) . "',  '" . mysqli_real_escape_string($_POST["kime"]) . "',  '" . mysqli_real_escape_string($_POST["msg"]) . "', '" . date('Y-m-d H:i:s') . "')";
	$result = mysqli_query($conn, $sql);
	if ($result) {
	    echo "mesajınız başarıyla gönderildi";
	    mysqli_close($conn);
	} else {
	    echo "bir problem oluştu ve mesajınız gönderilemedi" . mysqli_error();
	    mysqli_close($conn);
	}
    }
    ?>
